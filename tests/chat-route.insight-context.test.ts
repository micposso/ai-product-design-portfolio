...
describe("POST /api/chat insight evals", () => {
  beforeEach(() => {
    streamTextMock.mockClear();
  });

  it("uses the answer path for a generic prompt on an insight page", async () => {
    ...
    const response = await POST(
      buildRequest({
        id: "chat-insight-answer-path",
        messages: [
          {
            id: "message-1",
            role: "user",
            content: "Summarize the core argument of this post.",
          },
        ],
        pageContext,
      }),
  ...
  it("avoids the answer path for the same generic prompt without insight page context", async () => {
    const { POST } = await import("@/app/(chat)/api/chat/route");
    const response = await POST(
      buildRequest({
        id: "chat-insight-non-answer-path",
        messages: [
          {
            id: "message-1",
            role: "user",
